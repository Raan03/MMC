...
public class GameScreen : MonoBehaviour
{
    ...
    // all cards    
    List<Card> _aCards = new List<Card>();

    // cards in the grid
    Card[,] _aGrid = null;

    // All cards that have been flipped
    List<Card> _cardsFlipped = new List<Card>();
    ...
    {   
        ...
        for (int i = 0; i<totalSets; i++)
        {
            ...
            for (int j = 0; j<matchesPerCard; j++)
            {
                ...
                card = new Card(imageName, imageId);
                _aCards.Add(card);
